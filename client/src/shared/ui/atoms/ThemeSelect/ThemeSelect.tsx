...
export const ThemeSelect = ({ toggleTheme }: ThemeSelectProps) => {
  ...
  return (
    <Switch
      checked={enabled}
      onChange={setEnabled}
      className={`relative group inline-flex h-6 w-11 items-center rounded-full transition ${
    ...
    >
      <BsSunFill className="absolute left-1" color="#2b2b2b" />
      <span
      ...
      />
      <WiMoonWaningCrescent2 color="#ffffff" className=" absolute right-0.5" />
    </Switch>
  );
};
